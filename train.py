# -------------------------------------------------------------
#                  SemLoss+ScdLoss+SSL+RECOcontrasem
# -------------------------------------------------------------
...
from utils.pseudo_agu import batch_transform
from loss import OhemCrossEntropy2dTensor
...
train_log_dir = '/home/yyz/10185/Contrastive_SST/outdir/visualization' + TIMESTAMP
...
class Trainer:
    ...
    def validation(self, epoch):
        ...
        score_scd *= 100.0     
        dir_model = "outdir/0423_hrnet40_psp_scd+sem_st_contra/models"
        os.makedirs(dir_model, exist_ok=True)
        if score_scd >= self.previous_best:
            if self.previous_best != 0:
                model_path = "outdir/0423_hrnet40_psp_scd+sem_st_contra/models/%s_%s_%.2f.pth" % \
                             (self.args.model, self.args.backbone, self.previous_best)
                if os.path.exists(model_path):
                    os.remove(model_path)

            torch.save(self.model.module.state_dict(), "outdir/0423_hrnet40_psp_scd+sem_st_contra/models/%s_%s_%.2f.pth" %
