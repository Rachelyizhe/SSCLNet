...
class BaseNet(nn.Module):
    ...
    def base_forward(self, x1, x2):
        ...
        out1 = self.head(x1)# out1=[2,6,128,128]
        out2 = self.head(x2)

        out1 = F.interpolate(out1, size=(h, w), mode='bilinear', align_corners=False) # out1=[2,6,512,512]
        out2 = F.interpolate(out2, size=(h, w), mode='bilinear', align_corners=False)

        rep1 = F.interpolate(rep1, size=(h, w), mode='bilinear', align_corners=False) # rep1=[2,256,512,512]
        rep2 = F.interpolate(rep2, size=(h, w), mode='bilinear', align_corners=False)
        ...
        return out1, out2, rep1, rep2, out_bin.squeeze(1)
