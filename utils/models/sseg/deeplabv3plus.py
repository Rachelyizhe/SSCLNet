...
class DeepLabV3Plus(BaseNet):
    def __init__(self, backbone, pretrained, nclass, lightweight):
        ...
        self.reduce_mask_high = nn.Sequential(nn.Conv2d(high_level_channels * 3, high_level_channels, 1, bias=False),
                                        nn.BatchNorm2d(high_level_channels),
     
                                        nn.ReLU(True))
        self.reduce_mask_low = nn.Sequential(nn.Conv2d(low_level_channels * 3, low_level_channels, 1, bias=False),
        ...
        self.classifier_mask = nn.Conv2d(256, nclass, 1, bias=True)
    ...
    def base_forward(self, x1, x2):
        ...
        out_mask_4 = torch.abs(c1_4 - c2_4)
        out_mask_4 = self.head(out_mask_4)
        out_mask_4 = F.interpolate(out_mask_4, size=c1_1.shape[-2:], mode="bilinear", align_corners=False)
        out_mask_1 = torch.abs(c1_1 - c2_1)
        out_mask_1 = self.reduce_bin(out_mask_1)
        out_mask = torch.cat([out_mask_4, out_mask_1], dim=1)
        out_mask = self.fuse_bin(out_mask)
        out_mask = self.classifier_mask(out_mask)
        out_mask = F.interpolate(out_mask, size=(h, w), mode='bilinear', align_corners=False)
        ...
        return out_mask, out_bin.squeeze(1)
...
#         self.classifier_bin = nn.Conv2d(high_level_channels//8, 1, 1, bias=True)
#         self.classifier_mask = nn.Conv2d(high_level_channels//8, nclass, 1, bias=True)
