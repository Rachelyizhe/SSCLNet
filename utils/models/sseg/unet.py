...
class UNet(BaseNet):
    ...
    def base_forward(self, x1, x2):
        ...
        features1 = self.backbone.base_forward(x1) # x1=[2,600,128,128]
        features2 = self.backbone.base_forward(x2)
    
        out1 = self.decoder1(features1[-1], features1[-2])
        out1 = self.decoder2(out1, torch.abs(features1[-3] - features2[-3]))
        out1 = self.decoder3(out1, torch.abs(features1[-4] - features2[-4]))
        out1 = self.decoder4(out1, torch.abs(features1[-5] - features2[-5]))
        out1 = self.decoder5(out1)

        out_mask1 = self.classify(out1)
        ...
        out2_mask2 = self.classify(out2)


        return out_mask1, out2_mask2
